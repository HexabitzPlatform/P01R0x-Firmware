...
Module_Status Module_MessagingTask(uint16_t code, uint8_t port, uint8_t src, uint8_t dst, uint8_t shift) {
	...
	case CODE_H01R0_COLOR:
		if (cMessage[port - 1][shift] == 0) {
			...
			RGB_LED_setColor(cMessage[port - 1][1 + shift],
					cMessage[port - 1][2 + shift]);
		} else if (cMessage[port - 1][shift] == 1) {
			/* RGB color */
			RGB_LED_setRGB(cMessage[port - 1][1 + shift],
					cMessage[port - 1][2 + shift],
	...
	case CODE_H01R0_DIM:
		...
		repeat = ((uint32_t) cMessage[port - 1][10 + shift])
				...
				+ ((uint32_t) cMessage[port - 1][12 + shift] << 16)
				+ ((uint32_t) cMessage[port - 1][13 + shift] << 24);
		RGB_LED_dim(cMessage[port - 1][shift], cMessage[port - 1][1 + shift],
				period, dc, repeat);
